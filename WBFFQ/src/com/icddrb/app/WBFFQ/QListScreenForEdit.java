...
import com.icddrb.app.WBFFQ.adapters.QlistAdapter;
...
//	    filterText.addTextChangedListener(filterTextWatcher);
		...
		listentries.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View v, int pos,
				...
				listentries.smoothScrollToPosition(pos);
				
				
				// TODO Auto-generated method stub
				CommonStaticClass.currentSLNo = qslno.get(pos);
				if(CommonStaticClass.currentSLNo!=1){
//					if(CommonStaticClass.currentSLNo==31)// condition added by me
//					{
//						CommonStaticClass.mode = CommonStaticClass.EDITMODE;
//						Intent i = new Intent();
//						i.setClassName(CommonStaticClass.pName, CommonStaticClass.pName+".EditEntryMember");
//						startActivity(i);
//					}
...
//					}else 
					CommonStaticClass.mode = CommonStaticClass.EDITMODE;
					/*if(checkSelectionHasDataOrNot(CommonStaticClass.questionMap.get(CommonStaticClass.currentSLNo).getQvar(),CommonStaticClass.questionMap.get(CommonStaticClass.currentSLNo).getTablename()))
					{*/						
						startQuestion();
					/*}
					else{
						CommonStaticClass.showMyAlert(con, "Warning!!!", "This question has no data associated with it");
					}*/
				}else{
					CommonStaticClass.showMyAlert(con, "Warning!!!", "You can not edit particpant id");
				}
			}
...
//	}
	...
	protected boolean checkSelectionHasDataOrNot(String qvar, String tablename) {
		...
		try{			
			...
			if(mCursor1.getCount()>0){
				String  a = mCursor1.getString((mCursor1.getColumnIndex(qvar.toLowerCase())))+"";
				Log.e("aaaa",""+a);
		...
		try{
			...
			if(mCursor.moveToFirst()){
				do{
					...
					qdescbng.add(mCursor.getString((mCursor.getColumnIndex("Qdescbng"))));
					qdesceng.add(mCursor.getString((mCursor.getColumnIndex("Qdesceng"))));
	...
	private void startQuestion() {
		...
		Intent i = new Intent();
		i.setClassName(CommonStaticClass.pName, CommonStaticClass.pName+".questions.ParentActivity");
		startActivity(i);
	}
	private void setSkipper(){
        String sq = "Select q12 from tblMainQuesSC where dataid='"+CommonStaticClass.dataId+"'";
		Cursor mCursor1 = null;
		try{			
			mCursor1 = dbHelper.getQueryCursor(sq);
			if(mCursor1.moveToFirst()){
				do{		
					String val = mCursor1.getString((mCursor1.getColumnIndex("q12")))+"";
					if(val.length()>0 && !val.contains("-1") && !val.equalsIgnoreCase("null")){
		...
		String sql = "Select q204a from tblMainQues where dataid='"+CommonStaticClass.dataId+"'";
		mCursor1 = null;
		try{			
			mCursor1 = dbHelper.getQueryCursor(sql);
			if(mCursor1.moveToFirst()){
				do{		
					String val = mCursor1.getString((mCursor1.getColumnIndex("q204a")))+"";
					if(val.length()>0 && !val.contains("-1") && !val.equalsIgnoreCase("null")){
					int a = Integer.parseInt(val);
					if(a>=6&&a<12){

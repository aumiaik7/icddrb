...
import com.icddrb.app.Hbissaritac.db.DatabaseHelper;
import com.icddrb.app.Hbissaritac.questions.ParentActivity;

public class CommonStaticClass {

	public static String pName = "com.icddrb.app.Hbissaritac";
			...
			activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if(CommonStaticClass.questionMap.get(
							CommonStaticClass.currentSLNo)==null)
					{
						//activity.finish();
					}
	...
	public static boolean IsSeconDateGraterThanFirstDate(String date1, String date2)
	{
	...
	public static String CheckCursorValueWithNullHandler(Cursor c, String ColumnName) {
		if (c.getString(c.getColumnIndex(ColumnName)) != null) {
			if (c.getString(c.getColumnIndex(ColumnName)).length() > 0)

			{
				...
				if (c.getString(c.getColumnIndex(ColumnName)).toString().trim()
						.length() > 0) {
					return c.getString(c.getColumnIndex(ColumnName)).toString().trim();
	...
	public static void FillAutoCompleteTextView(ParentActivity act, DatabaseHelper dbHelper,
			String sql, AutoCompleteTextView spnr)
	{
	...
	public static String GetAutoCompleteValue(AutoCompleteTextView sp) {

		return ((sp).toString().substring(0, (sp)
				.toString().lastIndexOf(":"))).trim();
	...
	public static int DayDifferenceBackward(Calendar startDate, Calendar endDate ) {
		Calendar date = (Calendar) startDate.clone();
		int daysBetween = 0;
		//while (date.before(endDate)) {
	...
	public static int DayDifferenceBackwardWithMonth(Calendar startDate, Calendar endDate ) {
		...
		public static String Getcode(String name,String tableName, DatabaseHelper dbHelper) {
			// TODO Auto-generated method stub
			...
			try {
				...
				mCursor = dbHelper.getQueryCursor(String.format(
						"Select DrCode from "+tableName+" where Name = '%s'",
						name.trim()));
...
//						} while (mCursor.moveToNext());
		...
		public static String GetName(String code,String tableName, DatabaseHelper dbHelper) {
			// TODO Auto-generated method stub
			...
			try {
				...
				mCursor = dbHelper.getQueryCursor("Select Name from "+tableName+" where DrCode = '"+code+"'");

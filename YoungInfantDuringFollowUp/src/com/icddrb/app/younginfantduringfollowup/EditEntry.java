...
public class EditEntry extends BaseActivity{
	...
	private void loadQuestions() {
		...
		//String sql = "Select  dataid from tblMainQues order by CAST(substr(dataid,1,instr(dataid,'-')-1 ) as integer) asc,CAST(substr(dataid,instr(dataid,'-') +1) as integer) asc";
		Cursor mCursor = null;
		try{			
			mCursor = dbHelper.getQueryCursor(sql);
			if(mCursor.moveToFirst()){

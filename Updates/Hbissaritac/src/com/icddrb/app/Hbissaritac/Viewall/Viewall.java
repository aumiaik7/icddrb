package com.icddrb.app.Hbissaritac.Viewall;
...
public class Viewall extends Activity {
	...
	/*	String query = _intent.getString("sql");
		...
		// ((TextView)
		// findViewById(R.id.DisplayRowCount)).setText("Record(s) found:: " +
		// rowCount);
		...
		new Thread() {
			...
			public void run() {
				...
				// _sql =
				// " SELECT [PID],[Hh],[Slno],[Name], CASE [Sex] WHEN [Sex]=1 THEN 'Male' ELSE  'Female' END as [Sex], [Age], [BDate], [Rth], [MS], [Area], [SecBlock], [Road], [House], [Dist], [UPZ], [Vill], [Phone1], [Phone2], [Phone3], [Phone4], [GISID], [Ward], [Clust], [Block], [FaSize], [EnDate],[MotherName],[FatherName], [HeadName],[SpouseName], [Ucode], [UnionName], [VillK], [Vpart],[VillName], [Mohalla], [Bari],[Khana], [Project], [Location], rowid as _id from MemSCVB"
				// + query2;
				_sql = " SELECT dataid,HosCode,PatientID  ,HOSPID     ,PatientTY  ,RegDate    ,RegTime    ,Name       ,BDate      ,CASE [Sex] WHEN [Sex]=1 THEN 'Male' ELSE  'Female' END as [Sex],"
						+ "AgeYr      ,AgeMo      ,AgeDa      ,BDType     ,FaSpName   ,Ward       ,Area       ,SecBlock   ,Road       ,"
						+ "House      ,HH         ,SL         ,PID        ,Phone      ,Vill       ,UPZ        ,Dist       ,WT         ,HT         ,"
						+ "MUAC       ,DS17       ,DS18D      ,DS18H      ,DS19       ,DS20       ,DS21       ,DS22       ,DS23       ,DS24       ,"
						+ "DS25       ,DS26       ,DS27       ,DS28D      ,DS28H      ,DS29       ,DS30       ,DS31       ,DS32D      ,DS32H      ,"
...
/*				
				...
				_sql = " dataid,HosCode,PatientID  ,HOSPID     ,PatientTY  ,RegDate    ,RegTime    ,Name       ,BDate      ,CASE [Sex] WHEN [Sex]=1 THEN 'Male' ELSE  'Female' END as [Sex],"
						+ "AgeYr      ,AgeMo      ,AgeDa      ,BDType     ,FaSpName   ,Ward       ,Area       ,SecBlock   ,Road       ,"
						+ "House      ,HH         ,SL         ,PID        ,Phone      ,Vill       ,UPZ        ,Dist       ,WT         ,HT         ,"
						+ "MUAC       ,DS17       ,DS18D      ,DS18H      ,DS19       ,DS20       ,DS21       ,DS22       ,DS23       ,DS24       ,"
						+ "DS25       ,DS26       ,DS27       ,DS28D      ,DS28H      ,DS29       ,DS30       ,DS31       ,DS32D      ,DS32H      ,"
						...
						+ "DS51       ,DS52       ,DS53       ,DS54       ,DS55       ,DS56       ,DS57       ,DS58       ,DS59C      ,DS59P"
						+ ",DS60       ,DS61       ,DS62       ,DS63       ,DS64       ,DS65       ,DS66       ,DS67       ,"
						+ "DS68       ,DS69       ,ENTRYBY    ,EntryDate  ,EditBy     ,EditDate, rowid as _id from SCVBDS";// +
																															// query2;
	...
	private int GetRowCount(String sql) {
		...
		try {
			...
			if (mCursor.moveToFirst()) {
				do {
					rowCount = Integer.parseInt(mCursor.getString(mCursor
							.getColumnIndex("rowCount")));

...
import app.stoolsampleendline.CommonStaticClass;
...
public class LoginActivity extends BaseActivity {
	...
	private void loadGui() {
		...
		loginButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				//Dialog progressDialog = ProgressDialog.show(con, "Wait", "Updating options please wait");
	...
	/*	// TODO Auto-generated method stub
		...
		Log.e("userpass2",""+userpass2);
		if(userIdSelection.contains(userpass2)){
			CommonStaticClass.userSpecificId = userIdSelection;
			Intent intn = new Intent(LoginActivity.this, MenuScreen.class);
			startActivity(intn);
		}*/
		
		Log.e("userSpecificId",userSpecificId);
		String validationSql = "Select * from tblUser Where ID = '"+userSpecificId+"' AND Pwd = '"+userpass2+"'";
		Cursor mCursor = null;
		try{			
			mCursor = DatabaseHelper.getInstance(con).getQueryCursor(validationSql);
...
//				}
	...
	//private void loadData() {				
		/*users.add("1. Balay Chand Sikder");
		users.add("2. Mary Marget");
		users.add("3. Parimol Sarker");
		users.add("4. Dipali Rani Das");
		users.add("5. Sania Akter Dolly");
		users.add("6. Syeda Fatema Khatun");
		users.add("7. Md. Altafur Rahman");
		users.add("12. Syeda Luthfa Famida");
		users.add("13. Shajahan Ali");
	...
	private void loadData() {
		...
		Cursor mCursor = null;
		try{			
			mCursor = DatabaseHelper.getInstance(getApplicationContext()).getQueryCursor("Select * from tblUser where Active='Y' ORDER BY ID");
	...
	protected ArrayList<String> GetUser() {
		...
		Cursor mCursor = null;
		try {
			mCursor = DatabaseHelper.getInstance(con).getQueryCursor("Select * from tblUser where Active='Y' ORDER BY ID");
			if (mCursor.moveToFirst()) {
				do {
